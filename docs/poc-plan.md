...
1. **What does "done" look like to Marcus specifically, in numbers?** (★ top-3 if 5 minutes
   with priya) priya can describe the technical bar; only Marcus's number closes the deal.
   Without this, we optimize for a demo that impresses priya and stalls at the executive gate.
...
4. **What happened in the last IGA rollout that made it take 18 months and never finish?**
   priya was burned by this. If we repeat the same failure pattern without knowing what it
...
7. **How many people, realistically, will priya's team dedicate to this POC per week?**
...
8. **What does Northwind currently do about contractors who never appear in HR data?**
   ...
   need priya's own answer to before we invent one of our own that doesn't match her reality.
...
13. **Is there an existing SSO/IdP migration in flight that could change who "priya" even is
...
14. **What's priya's actual level of trust in her own HR data quality, independent of ours?**
...
| Review cycle time | A scoped Salesforce campaign completes reviewer decisions in days, not the 11-week baseline | priya's stated pain |
| No headcount added | Achieved without a net-new FTE on priya's team during the POC | Marcus's explicit "not adding headcount" constraint |
| Incident handled transparently | If something breaks during the POC (it will), priya and Dana can point to a specific, documented explanation, not a vague reassurance | priya's prior 18-month-rollout trauma |

## Staged plan, with entry/exit gates

| Stage | Entry gate | Exit gate | Target date |
|---|---|---|---|
| 0. Discovery | Kickoff scheduled | 15 questions answered, success criteria signed off by priya + Marcus | Day 2 |
| 1. Data foundation | Discovery complete | All 6 sources ingested; reconciliation report reviewed WITH priya, no unexplained quarantine | Day 5 |
| 2. Correlation & findings | Stage 1 exit | priya confirms the top 20 findings look directionally right (not perfect — directionally defensible) | Day 8 |
| 3. Lifecycle & review | Stage 2 exit | One real JML dry-run reviewed with priya; one scoped campaign (Salesforce) launched | Day 12 |
...
| priya's team can't dedicate review time | Medium | High | Confirm capacity in discovery (Q7); if capacity is thin, narrow campaign scope to the highest-value app (Salesforce) rather than all six sources |
